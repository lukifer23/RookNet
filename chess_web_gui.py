...
from engine.alpha_zero_engine import AlphaZeroEngine
...
class ChessAIServer:
    def __init__(self):
        ...
        self.device = torch.device(
            self.sys_config["device"]
            if self.sys_config["device"] != "auto"
        ...
        self.az_engine = (
            AlphaZeroEngine(self.model, config=CONFIG, device=self.device)
            if self.model
            else None
        )
    ...
    def _load_production_model(self) -> ChessTransformer:
        ...
        candidate_paths = [
            os.path.join(ckpt_dir, self.checkpoint_config["best_opponent_model"])
        ]
        # 2️⃣ Fallback: latest player model (exact name or any *_latest_player.pt)
        candidate_paths.append(
            os.path.join(ckpt_dir, self.checkpoint_config["latest_player_model"])
        )
        candidate_paths.extend(
            [str(p) for p in Path(ckpt_dir).glob("*_latest_player.pt")]
        )
        # 3️⃣ Fallback: newest iteration-specific checkpoint
        iter_ckpts = sorted(
            Path(ckpt_dir).glob("checkpoint_iter_*.pt"),
        ...
        candidate_paths.extend(
            [str(p) for p in Path(ckpt_dir).glob("*_best_opponent.pt")]
        )
        ...
        if model_path is None:
            logger.error(
                "FATAL: No suitable model checkpoint found in models/alpha_zero_checkpoints. The GUI will still run but neural engine options will be disabled."
            )
            return None
        ...
        try:
            ...
            logger.info(
                f"✅ Successfully loaded and prepared production model from {model_path}"
            )
            return model
        ...
        if not os.path.exists(stockfish_path):
            ...
            if not os.path.isabs(stockfish_path):
                ...
                if found_path:
                    ...
                    logger.warning(
                        f"Stockfish executable '{stockfish_path}' not found in system PATH or as a direct path. Please check config.v2.yaml."
                    )
                    self.stockfish_engine = None
                    return
            else:
                logger.warning(
                    f"Stockfish not found at path: {stockfish_path}. Please check config.v2.yaml."
                )
                self.stockfish_engine = None
                return
        try:
    ...
    def _setup_hopechess(self):
        ...
        try:
            if not HOPECHESS_BIN.exists():
                logger.info(
                    "Building HopeChess engine via zig ... this may take a minute"
                )
                try:
                    subprocess.run(
                        ["zig", "build", "-Drelease-safe"],
                        cwd=str(HOPECHESS_DIR),
                        check=True,
                    )
            ...
            proxy_path = (
                Path(__file__).resolve().parent / "tools" / "hopechess_proxy.py"
            )
    ...
    def evaluate_position(self, fen: str, stockfish_depth: int = 10):
        ...
        if self.model:
            try:
                board_tensor = (
                    torch.from_numpy(self.chess_env.board_to_tensor(chess.Board(fen)))
                    .float()
                    .unsqueeze(0)
                    .to(self.device)
                )
        ...
        if self.stockfish_engine:
            try:
                ...
                if not board.is_game_over():
                    ...
                    info = self.stockfish_engine.analyse(
    ...
    def get_hopechess_move(self, fen: str, strength: str = "medium"):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "HopeChess crashed or became unresponsive: %s. Restarting engine and retrying once.",
                    inner_err,
                )
                self._setup_hopechess()
                if not self.hopechess_engine:
            ...
            logger.info(
                ...
                strength,
                elapsed,
                reached_depth if reached_depth else "?",
                result.move,
            )
...
def api_get_ai_move():
    ...
    return jsonify(
        {"move": move, "evaluation": evaluation, "engine": "Native Transformer"}
    )
...
def api_evaluate_position():
    ...
    _, stockfish_eval, error = chess_ai.evaluate_position(
        fen, stockfish_depth=12
    )  # Use a decent depth
