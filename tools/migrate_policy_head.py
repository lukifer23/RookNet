...
def migrate_checkpoint(src_path: Path, dst_path: Path):
    ...
    if out_features > POLICY_SIZE:
        ...
        print(
            f"{src_path.name}: truncated policy head from {out_features} to {POLICY_SIZE}"
        )
    else:
        # Pad with zeros
        ...
        print(
            f"{src_path.name}: padded policy head from {out_features} to {POLICY_SIZE}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate checkpoints to 4096-output policy head"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        help="Destination path (default: append _4096.pt)",
    )
    ...
    dst = (
        ...
        if args.output
        else src.with_stem(src.stem + "_4096")
    )
