...
class ChessEnvironment:
    ...
    def select_move_with_temperature(
        self, board: chess.Board, policy: np.ndarray, temperature: float = 1.0
    ):
        ...
        """
        from utils.move_encoder import move_to_index
        ...
        probs = np.array(
            [policy[move_to_index(m)] for m in legal_moves], dtype=np.float64
        )
        if probs.sum() == 0:
