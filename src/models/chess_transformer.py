...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = (
            torch.bmm(attn, v).transpose(1, 2).contiguous().reshape(B, C, H, W)
        )  # B, C, H, W
...
class ChessTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.input_conv = nn.Conv2d(
            input_channels, cnn_channels, 3, padding=1, bias=False
        )
        ...
        self.cnn_blocks_list = nn.ModuleList(
            [
                ChessResidualBlock(
                    ...
                    use_attention=(
                        i >= cnn_blocks - 4
                    ),  # Add attention to last 4 blocks
                )
        ...
        self.transformer = ChessTransformerEncoder(
            d_model=cnn_channels, nhead=attention_heads, num_layers=transformer_layers
        )
...
def create_chess_transformer(config: str = "superhuman") -> ChessTransformer:
    ...
    if config not in configs:
        raise ValueError(
            f"Unknown config: {config}. Choose from {list(configs.keys())}"
        )
...
if __name__ == "__main__":
    ...
    print("🏆 Model ready for superhuman chess training!")
