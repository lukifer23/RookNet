...
class MCTSTransformerPlayer:
    ...
    def __init__(
        ...
        print("🏆 MCTS+Transformer Player initialized")
    ...
    def evaluate_position(
        ...
        if total_prior > 0:
            move_priors = {
                move: prior / total_prior for move, prior in move_priors.items()
            }
        else:
    ...
    def mcts_search(self, root_board: chess.Board) -> MCTSNode:
        ...
        if root.policy_priors:
            noise = np.random.dirichlet(
                [self.mcts_config.dirichlet_alpha] * len(root.policy_priors)
            )
            for i, move in enumerate(root.policy_priors.keys()):
                if move in root.children:
                    original_prior = root.children[move].prior
                    root.children[move].prior = (
                        1 - self.mcts_config.dirichlet_epsilon
                    ) * original_prior + self.mcts_config.dirichlet_epsilon * noise[i]
    ...
    def get_best_move(
        self, board: chess.Board, temperature: float = None
    ) -> chess.Move:
        ...
        if temperature == 0:
            ...
            best_move = max(
                root.children.keys(), key=lambda move: root.children[move].visits
            )
        else:
            # Stochastic selection with temperature
            visits = np.array(
                [root.children[move].visits for move in root.children.keys()]
            )
        ...
        if root.children[best_move].visits > 0:
            ...
            print(
                f"🔍 Move: {best_move} | Visits: {root.children[best_move].visits} | "
...
def create_ultimate_player(
    model_path: str = None, simulations: int = 800, device: str = "cuda"
) -> MCTSTransformerPlayer:
