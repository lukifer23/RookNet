...
class ChessTransformer(nn.Module):
    ...
    def forward(self, 
        ...
        if move_sequence is not None:
            ...
            move_embeddings = self.move_embedding(move_sequence) * math.sqrt(self.d_model)
            move_embeddings = self.pos_encoding(move_embeddings.transpose(0, 1)).transpose(0, 1)
...
class HybridChessModel(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.policy_head = nn.Linear(config.get("d_model", 512), config.get("vocab_size", 4096))
