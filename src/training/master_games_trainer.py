...
class MasterGamesTrainer:
    ...
    def parse_pgn_files(
        self, pgn_paths: List[str], max_positions: int = 100000
    ) -> List[MasterPosition]:
        ...
        for pgn_path in pgn_paths:
            ...
            try:
                with open(pgn_path, "r", encoding="utf-8", errors="ignore") as pgn_file:
                    game_positions = self._extract_positions_from_pgn(
                        pgn_file, max_positions - len(positions)
                    )
        ...
        print("   📈 Quality: Master-level annotated games")

        return positions[:max_positions]

    def _extract_positions_from_pgn(
        self, pgn_file, max_positions: int
    ) -> List[MasterPosition]:
            ...
            try:
                ...
                if move not in board.legal_moves:
                    ...
                    if failed_conversions <= 5:  # Debug first few failures
                        print(
                            f"      ⚠️ Illegal move: {pos.best_move} in position {pos.fen}"
                        )
                    continue
    ...
    def train_on_master_games(
        self, dataset: List[Tuple], epochs: int = 20
    ) -> Dict[str, List[float]]:
        ...
        for epoch in range(epochs):
            ...
            print(
                f"   🎯 Train Loss: {train_loss:.6f} | Policy Acc: {train_acc:.2f}% | Value MSE: {train_mse:.6f}"
            )
            print(
                f"   📊 Val Loss: {val_loss:.6f} | Policy Acc: {val_acc:.2f}% | Value MSE: {val_mse:.6f}"
            )
            ...
            if current_lr < 1e-7:
                print("   ⏹️ Early stopping - learning rate too low")
                break
    ...
    def _train_epoch(self, train_data: List[Tuple]) -> Tuple[float, float, float]:
        ...
        for i in range(0, len(train_data), self.batch_size):
            ...
            if batches % 100 == 0:
                ...
                print(
                    f"      Batch {batches}: Loss {total_loss/batches:.4f}, Acc {current_acc:.1f}%"
                )
