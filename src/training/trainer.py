...
class ChessTrainer:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize model
        self.model = create_model(config["model"]["type"], config["model"]).to(
            self.device
        )
        ...
        from torch.utils.tensorboard import (
            SummaryWriter,  # local import to avoid heavy dep if unused
        )
        ...
        if self.config.get("logging", {}).get("use_wandb", False):
            try:
                ...
                wandb_project = self.config["logging"].get(
                    "wandb_project", "ChessTrainer"
                )
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validation"):
                ...
                if (
                    ...
                    and len(self.model.forward.__code__.co_varnames) > 2
                ):
