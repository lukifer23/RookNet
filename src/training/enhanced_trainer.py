...
class ChessDataset(Dataset):
    ...
    def __init__(self, data: List[Dict[str, Any]]):
        ...
        self.moves = torch.LongTensor([item["move_index"] for item in data])

        self.evaluations = torch.FloatTensor([item["evaluation"] for item in data])
    ...
    def __getitem__(
        ...
        return (self.boards[idx], self.moves[idx], self.evaluations[idx], metadata)
...
class EnhancedTrainer:
    ...
    def setup_model(self) -> None:
        ...
        self.model = create_model(
            self.config["model"]["type"], self.config["model"]
        ).to(self.device)
    ...
    def _save_checkpoint(
        ...
        if is_best:
            ...
            checkpoint_path = (
                self.checkpoint_dir / f"final_model_{self.experiment_name}.pt"
            )
        else:
