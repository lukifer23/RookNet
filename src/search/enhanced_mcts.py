...
class MCTSNode:
    ...
    def select_child(self, config: MCTSConfig) -> "MCTSNode":
        ...
        best_child = max(
            available_children, key=lambda child: child.get_ucb_score(config.c_puct)
        )
        return best_child
...
class EnhancedMCTS:
    ...
    def search(
        self, board: chess.Board, config: Optional[MCTSConfig] = None
    ...
    def _expand_and_evaluate(self, node: MCTSNode) -> float:
        ...
        position_tensor = (
            torch.from_numpy(position_array).float().unsqueeze(0).to(self.device)
        )
    ...
    def _select_move(
        self, root: MCTSNode, config: MCTSConfig
    ) -> Tuple[chess.Move, Dict]:
    ...
    def _get_pv(
        self, root: MCTSNode, first_move: chess.Move, depth: int = 5
    ) -> List[chess.Move]:
